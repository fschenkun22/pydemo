...
## 第一步读合同号信息
from common.read_jobProducts_list import *
from common.read_contract_num_detail import *
from common.read_money import *
...
def get_full(contract_num):
    ...
    data = {}
    ## 调用读取函数
    contract_detail = read_contract_num_detail(contract_num)
    if contract_detail[0] == True:
        ...
        # print('JobID = ',JobID) 
        ## 拿JobID去找JobProducts
        JobList = read_job_products_list_by_JobID(JobID)
        ...
        if JobList[0] == True:
            ...
            ## 继续拿jobid读报价
        # print('debug money :')
        data['money'] = read_money_by_JobID(JobID)[2]
